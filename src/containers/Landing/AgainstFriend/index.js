...
const PlayAgainstFriend = () => {
  ...
  const joinMatchroom = useAction(matchroom.joinMatchroom);
  ...
  const { myMatchroom, started, mySide, myOpponent, me, ended } = useSelector(
    (state) => state.game
  );
  ...
  useEffect(() => {
    if (started) {
      console.log(myMatchroom);
      console.log(myMatchroom.timeControl);
      setTimeLimit(myMatchroom.timeControl);
      setStart(true);
      setSide(myMatchroom.side);
      ...
      setStart(false);
      joinMatchroom({ guest: true, token });
    }
  }, [started]);

  useEffect(() => {
    if (myMatchroom.winner) {
      setDraggable(false);
      setOpponentTimerActive(false);
      setYourTimerActive(false);
      if (myMatchroom.winner.id === me.id) {
        if (myMatchroom.reason === "surrender") {
          onOpponentSurrender(endGame, handleBack, false);
        } else if (myMatchroom.reason === "time_end") {
          onOpponentTimeFinish(endGame, handleBack, false);
        } else if (myMatchroom.reason === "mat") {
  ...
  const handleTimeChange = () => {
    if (started) {
      const lastMoveTime =
        myMatchroom.last_move_time || myMatchroom.lastMoveTime;
  ...
  const handleOponentTimeFinish = () => {
    win({ reason: "time_end", token });
  };

  const handleYourTimeFinish = () => {
    lose({ reason: "time_end", token });
  };

  const sendMoveRequest = (move) => {
    setMove({ matchId: myMatchroom.id, move });
  };
  ...
  const highlightCorrectMoves = (square) => {
    const chess = new Chess(myMatchroom.fen);
