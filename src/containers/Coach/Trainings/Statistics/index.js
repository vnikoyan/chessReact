...
import { groups, student, trainings } from "modules/coach/actions";
...
const CoachStudentStatistics = () => {
  ...
  const { currentTrainingStatistics, studentsTotal, studentsLoading } =
  ...
  const getTrainingStatistics = useAction(trainings.getStatistics);

  useEffect(() => {
    getGroups();
    getTrainingStatistics({ id });
  }, [getGroups]);

  const handlePageChange = (event, newPage) => {
    setPage(newPage);
    getTrainingStatistics({ id, page: newPage + 1 });
  };
                  ...
                  <CardContent>
                    <Grid container spacing={3}>
                      ...
                      <Grid item lg={12} md={12} xs={12}>
                        {studentsLoading ? (
                          <Loading />
                        ) : Boolean(currentTrainingStatistics.length) ? (
                          <TableContainer>
                            <Table>
                              <TableHead>
                                <TableRow>
                                  ...
                                  <TableCell>Solved tasks</TableCell>
                                  <TableCell>
                                    Solved tasks with mistake
                                  </TableCell>
                                  <TableCell>Avarage time for task</TableCell>
                                </TableRow>
                              </TableHead>
                              <TableBody>
                                {currentTrainingStatistics.map((student) => (
